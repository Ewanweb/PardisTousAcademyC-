...
public class GetPostsByTagSlugHandler : IRequestHandler<GetPostsByTagSlugQuery, PagedResult<PostListItemDto>>
{
    private readonly IRepository<Post> _postRepository;

    public GetPostsByTagSlugHandler(IRepository<Post> postRepository)
    {
        _postRepository = postRepository;
    }

    public async Task<PagedResult<PostListItemDto>> Handle(GetPostsByTagSlugQuery request, CancellationToken cancellationToken)
    {
        ...
        var query = _postRepository.Table
            .AsNoTracking()
            .Where(p => !p.IsDeleted && p.Status == PostStatus.Published && p.PostTags.Any(t => t.Tag.Slug == request.Slug))
