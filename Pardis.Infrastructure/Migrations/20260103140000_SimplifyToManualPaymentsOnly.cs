...
{
    /// <inheritdoc />
    public partial class SimplifyToManualPaymentsOnly : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // STEP 1: Archive existing non-manual payment attempts for safety
            migrationBuilder.Sql(@"
                -- Create archive table for non-manual payment attempts
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='PaymentAttempts_NonManual_Archive' AND xtype='U')
                BEGIN
                    SELECT * INTO PaymentAttempts_NonManual_Archive 
            ...
            migrationBuilder.Sql(@"
                ...
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='PaymentAttempts_Expired_Archive' AND xtype='U')
                BEGIN
                    SELECT * INTO PaymentAttempts_Expired_Archive 
                    FROM PaymentAttempts 
                ...
                -- Remove AwaitingReceiptUpload status (2) - merge into PendingPayment (1)
                UPDATE PaymentAttempts 
                SET Status = 1 -- PendingPayment
                WHERE Status = 2; -- AwaitingReceiptUpload
            ");

            // STEP 4: Consolidate ManualPaymentRequests into PaymentAttempts
            migrationBuilder.Sql(@"
                -- First, create Orders for ManualPaymentRequests that don't have them
                INSERT INTO Orders (Id, UserId, OrderNumber, TotalAmount, Currency, Status, CartSnapshot, CartId, CreatedAt, UpdatedAt)
                SELECT 
                    NEWID(),
                    mpr.StudentId,
                    ...
                    'IRR',
                    CASE 
                        WHEN mpr.Status = 2 THEN 2 -- Approved -> Completed
                        ELSE 1 -- Others -> PendingPayment
                    END,
                    '[]', -- Empty cart snapshot
                    NEWID(), -- Generate CartId
                    mpr.CreatedAt,
                    mpr.UpdatedAt
                FROM ManualPaymentRequests mpr
                WHERE NOT EXISTS (
                    SELECT 1 FROM PaymentAttempts pa 
                    WHERE pa.Id = mpr.Id
                );
                
                -- Then migrate ManualPaymentRequests to PaymentAttempts
                ...
                SELECT 
                    mpr.Id,
                    o.Id, -- OrderId from newly created orders
                    mpr.StudentId,
                    2, -- Manual
                    mpr.Amount,
                    CASE 
                    ...
                    mpr.AdminReviewedAt,
                    CASE 
                        WHEN mpr.Status = 2 THEN 'Approved' 
                        WHEN mpr.Status = 3 THEN 'Rejected' 
                        ELSE NULL 
                    END,
                    mpr.CreatedAt,
                    mpr.UpdatedAt,
                    'MAN-' + CONVERT(VARCHAR(36), mpr.Id)
                FROM ManualPaymentRequests mpr
                INNER JOIN Orders o ON o.OrderNumber = 'MAN-' + CONVERT(VARCHAR(36), mpr.Id)
                WHERE NOT EXISTS (
            ...
            migrationBuilder.Sql(@"
                ...
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='ManualPaymentRequests_Archive' AND xtype='U')
                BEGIN
                    SELECT * INTO ManualPaymentRequests_Archive 
            ...
            migrationBuilder.UpdateData(
                ...
                values: new object[] { "simplified-payment-system-2026", "AQAAAAIAAYagAAAAESimplifiedPaymentSystemHash2026" });
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                ...
                IF EXISTS (SELECT * FROM sysobjects WHERE name='ManualPaymentRequests_Archive' AND xtype='U')
                BEGIN
                    INSERT INTO ManualPaymentRequests SELECT * FROM ManualPaymentRequests_Archive;
                    DROP TABLE ManualPaymentRequests_Archive;
                END
                
                -- Restore non-manual PaymentAttempts from archive  
                IF EXISTS (SELECT * FROM sysobjects WHERE name='PaymentAttempts_NonManual_Archive' AND xtype='U')
                BEGIN
                    INSERT INTO PaymentAttempts SELECT * FROM PaymentAttempts_NonManual_Archive;
                    DROP TABLE PaymentAttempts_NonManual_Archive;
                END
            ");
        }
    }
